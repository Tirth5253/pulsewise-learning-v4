...
export default async function SubjectPage({ params }: { params: Promise<{ subjectId: string }> }) {
  ...
  return (
    <div className={styles.subjectPage}>
      <Sidebar subjects={data.subjects} lessons={data.lessons} currentSubjectId={subjectId} />
      ...
      <div className={styles.mainContent}>
        ...
        <div className={styles.subjectHeader}>
          ...
          <div className={styles.subjectInfo}>
            ...
            <ProgressTracker subjects={[subject]} lessons={lessons} currentSubjectId={subject.id} />
          </div>
        </div>

        <div className={styles.lessonsGrid}>
          {lessons.map((lesson, index) => (
            <Link key={lesson.id} href={`/learning/${subjectId}/${lesson.id}`} className={styles.lessonLink}>
              <Card className={styles.lessonCard}>
                <div className={styles.lessonNumber}>Lesson {index + 1}</div>
                <h3 className={styles.lessonTitle}>{lesson.title}</h3>
                <p className={styles.lessonDescription}>{lesson.description}</p>
                <div className={styles.lessonMeta}>
                  <span className={styles.cardCount}>{lesson.totalCards} cards</span>
